...
app.post('/upload2', (req, res) => {
    ...
    fs.writeFileSync(`./upload/${filename}`, file);
...
btn.addEventListener('click', () => {
    ...
    fileInput.addEventListener('change', function () {
        ...
        let file = this.files[0];
        // 调用切片函数，得到我们的请求队列
        let requestList = createRequestList(file);
        // 调用发送切片的函数
        sendSlice(requestList);
    })
})
```
...
// 创建请求队列的函数
function createRequestList(file) {
    // 1. 配置切片的个数，这里我选择切割为100份
    let sliceLength = 100;
    
    // 2. 得到每一个切片的大小 （总大小/100）
...
app.post('/upload3', multer().single('chunk'), (req, res) => {
    ...
    try { // try-catch是为了防止前端传递的文件名错误
        chunkName = filename.match(/^(.+)_\d+(\.\w+)/)[1];
    } catch (error) {
    ...
    // 6. 写入文件
    fs.writeFile(path.resolve(__dirname, chunkPath, filename), req.file.buffer, (err) => {
        if (err) throw err;
...
app.get('/merge', (req, res) => {
    ...
    // 5. 读取目录下的所有文件，这个目录下就存放着要合并的所有切片
    let dirs = fs.readdirSync(chunkPath);

    // 6. 对切片文件进行顺序排列
    dirs.sort((a, b) => {
        // 7. 提取切片索引的正则表达式，如demo_66.mp4 => 66
    ...
    // 9. 遍历所有文件，并进行合并
    dirs.forEach(file => {
        // 10. 得到切片文件的完整路径，如 C:upload-file/upload/demo/demo_66.mp4
        let fullpath = path.resolve(__dirname, chunkPath, file);
        
        // 11. 写入文件
        fs.appendFileSync(`./upload/${filename}`, fs.readFileSync(fullpath));
        
        // 12. 当一个切片写入完成后，则删除该切片
